...
typedef struct miniosc_t miniosc;
...
miniosc * minioscInit( unsigned short portin, unsigned short portout, const char * addressout, int * minioscerrorcode );
...
int minioscPoll( miniosc * osc, int timeoutms, void (*callback)( const char * address, const char * type, const void ** parameters ) );
...
static int minioscEncodeInternal( char * buffer, char ** bptr, int mob, const char * address, const char * type, va_list ap )
{
	...
	err = _minioscAppend( buffer, bptr, MINIOSCBUFFER, (int)strlen( address ) + 1, address );
	if( err ) return err;
	...
	for( t = type+1; *t; t++ )
	{
		...
		case 'r':
		{
			...
			i = htonl( i );
			err = _minioscAppend( buffer, bptr, MINIOSCBUFFER, 4, (const char *)&i );
			if( err ) return err;
			break;
		}
		case 'f':
		{
			...
			*fi = htonl( *fi );
			...
			err = _minioscAppend( buffer, bptr, MINIOSCBUFFER, 4, (const char *)&f );
			if( err ) return err;
			break;
		}
		case 's':
		{
			...
			int sl = (int)strlen( st ) + 1;
			err = _minioscAppend( buffer, bptr, MINIOSCBUFFER, sl, st );
			if( err ) return err;
			break;
		}
		case 'b':
		{
			...
			err = _minioscAppend( buffer, bptr, MINIOSCBUFFER, 4, (const char *)&lensend );
			if( err ) return err;

			err = _minioscAppend( buffer, bptr, MINIOSCBUFFER, len, (const char *)st );
			if( err ) return err;
			break;
		}
		case 'F':
...
int minioscBundle( mobundle * mo, const char * address, const char * type, ... )
{
	...
	if( ret == 0 )
	{
		...
		int len = (int)(mo->bundleplace - (char*)bpl - 4);
		if( len )
		{
			(*bpl) = htonl( len );
		}
...
int minioscSendBundle( miniosc * osc, mobundle * mo )
{
	...
	int sendlen = (int)(mo->bundleplace - mo->bundledata);
...
static int minioscProcess( char * buffer, char ** eptr, int r, void (*callback)( const char * address, const char * type, const void ** parameters ) )
{
	...
	const void * parameters[MINIOSCBUFFER/4]; // Worst-case scenario: all blobs.
	int p = 0;
	const char * ct;
	...
	for( ct = type+1; *ct; ct++ )
	{
		...
		case 's':
		{
			...
			if( sl == 0 )
			{
				parameters[p] = (const void*)"\0\0\0\0";
				*eptr += 4;
			}
		...
		case 'b':
		{
			...
			*eptr += 4;
			...
			int sl = _minioscGetQBL( buffer, eptr, r );
			if( sl < 0 ) return MINIOSC_ERROR_PROTOCOL;
			if( sl == 0 ) parameters[p++] = (const void*)"\0\0\0\0";
			else parameters[p++] = st;
			break;
		}
		case 'T':
...
int minioscPoll( miniosc * osc, int timeoutms, void (*callback)( const char * address, const char * type, const void ** parameters ) )
{
	...
	{
		int err = minioscProcess( buffer, &eptr, r, callback );
		if( err < 0 ) return err;
		rxed = 1;
	}
	return rxed;
